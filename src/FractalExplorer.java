    ...
    private void drawFractal(){
        ...
        for(int x = 0; x < displaySize; x++){
            for(int y = 0; y < displaySize; y++){
                ...
                int m = generator.numIterations(xCoord, yCoord);
                if(m == -1){
                    display.drawPixel(x, y, 0);
                }
                else{
                    float g = 0.7f + (float) m/200f;
                    int rgbColor = Color.HSBtoRGB(g, 1f, 1f);
            ...
            if(e.getActionCommand().equals("Reset")) {
                ...
                if(t == JFileChooser.APPROVE_OPTION){
                    try{
                        ...
                        JOptionPane.showMessageDialog(display, ee.getMessage(),"Ne mojno save image!", JOptionPane.ERROR_MESSAGE);
                    }
                }
            }
            else {
